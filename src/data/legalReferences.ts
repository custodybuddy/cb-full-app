...
import {
  JURISDICTIONS as JURISDICTION_METADATA,
...
// ----------------------------
// Federal and provincial/territorial references
// ----------------------------
...
export const CANADA_FEDERAL_STATUTES = {
  divorceAct: {
    ...
    bestInterests: 'https://laws-lois.justice.gc.ca/eng/acts/d-3.4/page-3.html#h-1172510',
    familyViolence: 'https://laws-lois.justice.gc.ca/eng/acts/d-3.4/page-3.html#h-1172571',
    familyViolenceCoerciveControl: 'https://laws-lois.justice.gc.ca/eng/acts/d-3.4/page-1.html#h-1172378',
    familyViolenceDef: 'https://laws-lois.justice.gc.ca/eng/acts/d-3.4/page-1.html#h-1172378',
    relocation: 'https://laws-lois.justice.gc.ca/eng/acts/d-3.4/page-4.html#h-1172637',
  },
  criminalCode: {
    harassment: 'https://laws-lois.justice.gc.ca/eng/acts/c-46/section-264.html',
    intimidation: 'https://laws-lois.justice.gc.ca/eng/acts/c-46/section-423.html',
    coerciveControlRelated: 'https://laws-lois.justice.gc.ca/eng/acts/c-46/section-264.1.html',
  },
};
...
export const CANADA_PROVINCIAL_ACTS: Record<string, { familyLawAct?: string; bestInterests?: string; [key: string]: string | undefined }> =
  {
    ontario: {
      ...
      bestInterests: 'https://www.ontario.ca/laws/statute/90c12#BK9',
    },
    'british columbia': {
      familyLawAct: 'https://www.bclaws.gov.bc.ca/civix/document/id/complete/statreg/11025_01',
      bestInterests: 'https://www.bclaws.gov.bc.ca/civix/document/id/complete/statreg/11025_03#section37',
    },
    alberta: {
...
// Provincial/territorial statute refs for rendering badges/links (normalized keys)
...
// Legacy coercive control mappings for U.S. states
export const US_COERCIVE_CONTROL_LAWS: Record<string, { coerciveControl: string }> = {
  california: {
    coerciveControl: 'https://leginfo.legislature.ca.gov/faces/codes_displaySection.xhtml?sectionNum=6320.&lawCode=FAM',
  },
  connecticut: {
    coerciveControl: 'https://www.cga.ct.gov/current/pub/chap_815e.htm#sec_46b-1',
  },
  hawaii: {
    coerciveControl: 'https://www.capitol.hawaii.gov/hrscurrent/Vol12_Ch0501-0588/HRS0586/HRS_0586-0001.htm',
  },
  washington: {
    coerciveControl: 'https://app.leg.wa.gov/rcw/default.aspx?cite=7.105.010',
  },
  colorado: {
    coerciveControl: 'https://leg.colorado.gov/bills/hb24-1088',
  },
};
...
// Friendly display list for dropdowns/UI; keep metadata intact for lookups.
export const JURISDICTION_NAMES = JURISDICTION_METADATA.map((j) => j.region);

export const DEFAULT_JURISDICTION: JurisdictionMetadata =
  JURISDICTION_METADATA.find((j) => j.id === 'ontario') ?? JURISDICTION_METADATA[0];
...
export function getStatutesForJurisdiction(j: JurisdictionInfo, behaviorType?: BehaviorType): StatuteRef[] {
  const statutes: StatuteRef[] = [];
  const seen = new Set<string>();
  const add = (s: StatuteRef) => {
    if (!s.link || seen.has(s.link)) return;
    seen.add(s.link);
    statutes.push(s);
  };

  if (j.country === 'CA') {
    add({ label: 'Divorce Act — Family Violence (s.2(1))', link: CANADA_FEDERAL.divorceAct.definitionFamilyViolence });
    add({ label: 'Divorce Act — Best Interests (s.16(4))', link: CANADA_FEDERAL.divorceAct.bestInterests });

    const provincial = CANADA_PROVINCES[normalizeJurisdiction(j.id)];
    if (provincial) provincial.forEach(add);

    add({ label: 'Criminal Code — Intimidation (s.423)', link: CANADA_FEDERAL.criminalCode.intimidation });
  }
  ...
  // Optional behavior-based add-ons for Canada (from glossary)
  if (behaviorType && j.country === 'CA') {
    const glossaryEntry = COERCIVE_CONTROL_GLOSSARY[behaviorType];
    if (glossaryEntry?.canadaStatute) {
      add({ label: 'Criminal Code', link: glossaryEntry.canadaStatute });
    }
  }
...
export const mapAllegationToStatutes = (behaviorType: string, jurisdiction: JurisdictionInfo): StatuteRef[] => {
  ...
  // Canada federal anchors
  statutes.push(
    {
      label: 'Divorce Act — Family Violence (s. 2(1))',
      link: CANADA_FEDERAL_STATUTES.divorceAct.familyViolenceCoerciveControl,
    },
    {
      label: 'Divorce Act — Best Interests (s.16)',
      link: CANADA_FEDERAL_STATUTES.divorceAct.bestInterests,
    }
  );

  // Criminal Code reference from glossary (harassment / intimidation / threats)
  if (glossaryEntry?.canadaStatute) {
    statutes.push({
      label: 'Criminal Code',
  ...
  // Provincial / Territorial overlays
  if (jurisdiction.country === 'CA') {
    const province = CANADA_PROVINCIAL_ACTS[jurisdiction.id];
    if (province?.familyLawAct) {
      statutes.push({
        label: `${jurisdiction.region} Family Law Act`,
        link: province.familyLawAct,
      });
    }
    if (province?.bestInterests) {
      statutes.push({
        label: 'Best Interests Factors',
        link: province.bestInterests,
      });
    }
  }

  // U.S. coercive control laws
  if (jurisdiction.country === 'US') {
    const state = US_COERCIVE_CONTROL_LAWS[jurisdiction.id];
    if (state) {
      statutes.push({
