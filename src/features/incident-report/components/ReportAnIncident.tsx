...
import { useIncidentReportState, useIncidentReportActions } from '../useIncidentReporter';
...
const ReportAnIncident: React.FC<{ isOpen: boolean }> = ({ isOpen }) => {
    ...
    const storedJurisdictionRef = useRef<string | null>(null);

    // Reset state when the modal is closed, but cancel if reopened quickly
    useEffect(() => {
        if (!isOpen) {
    ...
    // Prefill jurisdiction from localStorage if present
    useEffect(() => {
        try {
            storedJurisdictionRef.current = localStorage.getItem('cb_jurisdiction');
        } catch {
            storedJurisdictionRef.current = null;
        }
        if (storedJurisdictionRef.current) {
            setIncidentData(prev => ({ ...prev, jurisdiction: storedJurisdictionRef.current || '' }));
        }
    }, [setIncidentData]);
                    ...
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                try {
                                    localStorage.setItem('cb_jurisdiction', e.target.value);
                                } catch {
