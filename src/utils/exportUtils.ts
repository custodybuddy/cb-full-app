...
import type { AllegationSummary, JurisdictionInfo } from '@/types/legal';
...
export const exportElementAsPDF = async (element: HTMLElement, filename:string, headerText?: string): Promise<void> => {
    try {
        ...
        const canvas = await html2canvas(element, {
            ...
            backgroundColor: '#ffffff', // print-friendly background
            useCORS: true,
...
// Markdown-to-PDF friendly summary builder
export function buildMarkdownSummary(incident: AllegationSummary, j: JurisdictionInfo): string {
    const statutes = incident.statutes.map(s => `- **${s.label}**\n  ${s.link}`).join('\n');
