...
export const useFocusTrap = (
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            const firstElement = focusableElements[0] || modalNode;
            const lastElement = focusableElements[focusableElements.length - 1] || modalNode;

            // Move focus to the first focusable element in the modal (fallback to container)
            firstElement?.focus();
